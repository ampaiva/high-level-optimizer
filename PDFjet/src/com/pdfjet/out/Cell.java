...
public class Cell {
    ...
    public float top_padding = 2f;

    public float bottom_padding = 2f;

    public float left_padding = 2f;

    public float right_padding = 2f;

    public float lineWidth = 0.2f;

    public int background = -1;

    public int pen = Color.black;

    public int brush = Color.black;

    public int properties = 0x000F0001;
    ...
    protected void paint(Page page, float x, float y, float w, float h) throws Exception {
        ...
        if (point != null) {
            ...
            if (point.getURIAction() != null) {
                page.annots.add(new Annotation(point.getURIAction(), null, point.x - point.r, page.height - (point.y - point.r), point.x + point.r, page.height - (point.y + point.r)));
            }
    ...
    private void drawText(Page page, float x, float y, float cell_w) throws Exception {
        ...
        if (getTextAlignment() == Align.RIGHT) {
            ...
            if (compositeTextLine == null) {
                x_text = x + this.left_padding + (((cell_w - (left_padding + right_padding)) - font.stringWidth(text)) / 2);
                page.drawString(font, text, x_text, y_text);
                if (getUnderline()) {
                    underlineText(page, font, text, x_text, y_text);
                }
                ...
                x_text = x + this.left_padding + (((cell_w - (left_padding + right_padding)) - compositeTextLine.getWidth()) / 2);
                compositeTextLine.setPosition(x_text, y_text);
                compositeTextLine.drawOn(page);
            }
        ...
        if (uri != null) {
            ...
            page.annots.add(new Annotation(uri, null, x_text, (page.height - y_text) + font.descent, x_text + w, (page.height - y_text) + font.ascent));
        }
    }
